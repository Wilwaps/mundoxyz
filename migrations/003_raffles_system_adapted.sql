-- ================================================================
-- SISTEMA DE RIFAS COMPLETO - MIGRACIÓN 003 (ADAPTADA)
-- ================================================================
-- Autor: Cascade AI
-- Fecha: 2025-10-28
-- Descripción: Sistema completo de rifas adaptado a estructura existente
-- NOTA: Trabaja con tabla raffles existente, añade columnas faltantes

-- ================================================================
-- 1. ADAPTAR TABLA RAFFLES EXISTENTE
-- ================================================================
DO $$
BEGIN
    -- Verificar si la tabla raffles existe y adaptar columnas
    IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'raffles') THEN
        ...
        -- Mapear columnas existentes a nuevo sistema
        -- entry_price_fire -> cost_per_number (usar existing)
        -- numbers_range -> ya existe (integer)
        -- visibility -> type (public/private)
        -- host_meta -> datos de contacto y empresa
        -- prize_meta -> detalles del premio
        
        -- Actualizar type basado en visibility existente
        UPDATE raffles 
        SET type = CASE 
            ...
            ELSE 'public'
        END
        WHERE type IS NULL OR type = 'public';
        ...
        IF NOT EXISTS (SELECT FROM pg_indexes WHERE tablename = 'raffles' AND indexname = 'idx_raffles_scheduled_close') THEN
...
CREATE INDEX IF NOT EXISTS idx_raffle_purchases_purchase_type ON raffle_purchases(purchase_type, status);
...
CREATE INDEX IF NOT EXISTS idx_raffle_tickets_number ON raffle_tickets(number_id);
...
SELECT 
    ...
    r.mode,
    r.type,
    r.visibility as original_visibility,
    r.status,
    COALESCE(r.entry_price_fire, 0) as cost_per_number,
...
-- FINALIZACIÓN
-- ================================================================
-- La migración adaptada está lista para ser ejecutada en Railway PostgreSQL
-- Trabaja con la estructura existente y añade las nuevas funcionalidades
-- ================================================================
