...
-- Cuentas abiertas por mesa en el POS (restaurante)
CREATE TABLE IF NOT EXISTS store_table_tabs (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    store_id UUID NOT NULL REFERENCES stores(id) ON DELETE CASCADE,
    table_label VARCHAR(50) NOT NULL, -- e.g., 'Mesa 1'
    cart_items JSONB NOT NULL DEFAULT '[]', -- Snapshot del carrito de la mesa
    total_usdt DECIMAL(20, 2) NOT NULL DEFAULT 0,
    version BIGINT NOT NULL DEFAULT 1, -- Control de concurrencia optimista
    locked_by UUID REFERENCES users(id), -- Opcional: quién está facturando la mesa
