...
async function main() {
  ...
  const sqlPath = path.resolve(__dirname, '../db/migrations/082_create_production_recipes_and_batches.sql');
  ...
  try {
    ...
    console.log('✅ Conectado. Ejecutando migración 082_create_production_recipes_and_batches.sql');

    await client.query('BEGIN');
    await client.query(sql);
    await client.query('COMMIT');

    console.log('✅ Migración 082 aplicada correctamente.');
  } catch (error) {
    try {
