...
async function main() {
  ...
  const sqlPath = path.resolve(__dirname, '../db/migrations/050_alter_telegram_link_sessions_link_token.sql');
  ...
  try {
    ...
    console.log('✅ Conectado. Ejecutando migración 050_alter_telegram_link_sessions_link_token.sql');

    await client.query('BEGIN');
    await client.query(sql);
    await client.query('COMMIT');

    console.log('✅ Migración 050 aplicada correctamente.');
  } catch (error) {
    try {
