 ...
 * Abandonar juego manualmente (sin reembolso)
 */
router.post('/rooms/:code/abandon', verifyToken, async (req, res) => {
  try {
    const { code } = req.params;
    
    const roomResult = await query(`
      SELECT id, status, host_id FROM bingo_rooms 
      WHERE code = $1 AND status = 'playing'
    `, [code]);
    
    if (!roomResult.rows.length) {
      return res.status(404).json({ error: 'Sala no encontrada o no en juego' });
    }
    
    const room = roomResult.rows[0];
    const isHost = room.host_id === req.user.id;
    
    // Eliminar jugador de la sala (pierde su inversión)
    await query(`
      DELETE FROM bingo_room_players 
      WHERE room_id = $1 AND user_id = $2
    `, [room.id, req.user.id]);
    
    // Si es el host, marcar sala como abandonada
    if (isHost) {
      await query(`
        UPDATE bingo_rooms 
      ...
      // Notificar a Admin/Tote
      await BingoAbandonmentService.markRoomAsAbandoned(room.id, {
        code,
      ...
      logger.warn(`⚠️  Host ${req.user.username} abandonó sala ${code} voluntariamente`);
    } else {
      logger.info(`Usuario ${req.user.username} abandonó sala ${code} voluntariamente`);
    }
    ...
    logger.error('Error en abandono de juego:', error);
    res.status(500).json({ error: error.message });
  }
});
 ...
 * Admin/Tote toma control de sala abandonada
 */
router.post('/rooms/:code/take-control', verifyToken, async (req, res) => {
  try {
    const { code } = req.params;
    ...
    if (!userResult.rows.length || userResult.rows[0].tg_id !== '1417856820') {
      return res.status(403).json({ error: 'Solo Admin/Tote puede tomar control de salas abandonadas' });
    }
    
    const roomResult = await query(`
      SELECT id FROM bingo_rooms 
      WHERE code = $1 AND host_abandoned = TRUE AND status = 'playing'
    `, [code]);
    
    if (!roomResult.rows.length) {
      return res.status(404).json({ error: 'Sala no encontrada o no está abandonada' });
    }
    
    const result = await BingoAbandonmentService.takeControlOfAbandonedRoom(
    ...
    logger.error('Error tomando control de sala:', error);
    res.status(500).json({ error: error.message });
  }
});
 ...
 * Listar salas abandonadas (solo Admin/Tote)
 */
router.get('/abandoned-rooms', verifyToken, async (req, res) => {
  try {
    // Verificar que el usuario es Admin/Tote
    const userResult = await query(`
      SELECT tg_id FROM users WHERE id = $1
    ...
    if (!userResult.rows.length || userResult.rows[0].tg_id !== '1417856820') {
      return res.status(403).json({ error: 'Solo Admin/Tote puede ver salas abandonadas' });
    }
    ...
    const abandonedRooms = await query(`
      SELECT 
        ...
        r.currency,
        r.host_abandoned,
        r.substitute_host_id,
        r.abandonment_detected_at,
        r.host_last_activity,
        EXTRACT(EPOCH FROM (NOW() - r.host_last_activity)) as inactive_seconds,
        u.username as original_host,
        COUNT(p.user_id) as player_count
      FROM bingo_rooms r
      JOIN users u ON u.id = r.host_id
      LEFT JOIN bingo_room_players p ON p.room_id = r.id
      WHERE r.host_abandoned = TRUE AND r.status = 'playing'
      GROUP BY r.id, u.username
      ORDER BY r.abandonment_detected_at DESC
    `);
    
    res.json({
      success: true,
      rooms: abandonedRooms.rows
    });
    
  } catch (error) {
    logger.error('Error obteniendo salas abandonadas:', error);
    res.status(500).json({ error: error.message });
  }
});
