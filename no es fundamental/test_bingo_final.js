...
async function createFreeRoom() {
    try {
        log('🏗️ Creando sala GRATIS de prueba final...');
        const roomConfig = {
            roomName: 'SALA TEST FINAL FUNCIONAL',
            roomType: 'public',
            currency: 'fires',
            numbersMode: 75,
            victoryMode: 'line',
...
async function buyCards() {
    try {
        ...
        log(`   IDs: ${response.data.card_ids?.join(', ') || 'N/A'}`);
        return true;
    } catch (error) {
...
async function getRoomDetails() {
    try {
        ...
        log(`   Cartones usuario: ${room.user_cards?.length || 0}`);
        
        if (room.user_cards && room.user_cards.length > 0) {
            const firstCard = room.user_cards[0];
            log(`   Primer cartón: ${JSON.stringify(firstCard.numbers.slice(0, 15))}...`);
        }
...
async function markReady() {
    try {
        ...
        log(`   Estado sala: ${response.data.room.status}`);
        return true;
    } catch (error) {
...
async function startGame() {
    try {
        ...
        log(`   Números totales: ${response.data.room.total_numbers}`);
        return true;
    } catch (error) {
...
async function testCompleteBingoFlow() {
    log('🚀 INICIANDO PRUEBA COMPLETA BINGO FINAL', 'SUCCESS');
    log('=' .repeat(60));
    ...
    // 9. Resumen final
    log('=' .repeat(60));
    log('📊 RESUMEN PRUEBA BINGO COMPLETA', 'SUCCESS');
    ...
    log(`✅ Backend estable: Todos los endpoints funcionando`);
    log('🎉 SISTEMA BINGO 100% FUNCIONAL', 'SUCCESS');
}

// Ejecutar prueba
testCompleteBingoFlow().catch(error => {
